...
foreach ($fileNames as $name)
{
	...
	test_false(file_exists($abspath), "file/del failed to remove $abspath, before commit");
}
...
{
	$abspath = "$repopath/$name";
	test_false(file_exists($abspath), "file/del failed to remove $abspath, after commit");
}
